...
export class DemoDayApp {
  ...
  #renderCaptainCabin(slot: ProvisionedWeek, configurationEventId: string, configuration: WeekConfigurationV1): string {
    ...
    return `<section class="week-subsection"><h2>Captain’s private workspace</h2><section class="panel"><h3>Proposal intake</h3><p>${configuration.intake_open ? "Open" : "Closed"}</p></section>
      <h3>Private proposal inbox</h3>${this.#cabinLoading.has(scope) ? `<p><span class="spinner"></span> Decrypting…</p>` : proposalCards || `<p>No valid encrypted proposals.</p>`}
